...
def main():
    with psycopg2.connect(DSN) as conn, conn.cursor() as cur:
        insert_members(cur, os.path.join("/opt/airflow","data","members_seed.csv"))
        insert_books(cur, os.path.join("/opt/airflow","data","books_seed.csv"))
        insert_loans_random(cur, n=12)
        conn.commit()
    print("Inserted: members, books, loans")
